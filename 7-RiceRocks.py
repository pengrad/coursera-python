...
class Sprite:
    ...
    def update(self):
        ...
        # update age, return true if sprite must be removed
        self.age += 1
...
def rock_spawner():
    if len(rock_group) >= 12 or not started: return
    rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
    rock_avel = random.random() * .2 - .1
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]    
    
    # check rock to spawn not too close to the ship
    delta = 100
    while dist(my_ship.get_position(), rock_pos) < (my_ship.get_radius() + asteroid_info.get_radius() + delta):
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
